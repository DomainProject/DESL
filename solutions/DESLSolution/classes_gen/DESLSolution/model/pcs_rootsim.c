...
static void fading_recheck(struct lp_state_type *state, struct rng_t * ctx)
{
  ...
  while(ch != NULL) {
    ch->sir_data->fading = Expent(ctx, 1.0);
    ch = list_next(ch);
  }
...
;
    ...
    {
      c->sir_data->fading = Expent(ctx, 1.0);
...
;
       ...
       */
      double timestamp_LP_INIT = ((double)((20 * Random(state->ctx))));
       ...
       */
      timestamp_LP_INIT = ((double)((pcs_FADING_RECHECK_FREQUENCY * Random(state->ctx))));
    ...
    case START_CALL:
      ...
      else
      {
        ...
        switch (pcs_DURATION_DISTRIBUTION)
        {
          case pcs_UNIFORM:
          {
            new_event_content_START_CALL.call_term_time = now + ((double)((ta_duration * Random(state->ctx))));
            break;
          }
          case pcs_EXPONENTIAL:
          {
            new_event_content_START_CALL.call_term_time = now + ((double)((Expent(state->ctx, ta_duration))));
            break;
          }
          default: {
            new_event_content_START_CALL.call_term_time = now + ((double)((5 * Random(state->ctx))));
          }
        }
        ...
        switch (pcs_CELL_CHANGE_DISTRIBUTION)
        {
          case pcs_UNIFORM:
          {
            handoff_time_START_CALL = now + ((double)((ta_change) * Random(state->ctx)));
            break;
          }
          case pcs_EXPONENTIAL:
          {
            handoff_time_START_CALL = now + ((double)((Expent(state->ctx, ta_change))));
            break;
          }
          default: {
            handoff_time_START_CALL = now + ((double)((5 * Random(state->ctx))));
          }
        }
      ...
      switch (pcs_DISTRIBUTION)
      {
        case pcs_UNIFORM:
        {
          timestamp_START_CALL = now + ((double)((state->ta * Random(state->ctx))));
          break;
        }
        case pcs_EXPONENTIAL:
        {
          timestamp_START_CALL = now + ((double)((Expent(state->ctx, state->ta))));
          break;
        }
        default: {
          timestamp_START_CALL = now + ((double)((5 * Random(state->ctx))));
        }
      }
    ...
    case HANDOFF_RECV:
      ...
      else
      {
        ...
        switch (pcs_CELL_CHANGE_DISTRIBUTION)
        {
          case pcs_UNIFORM:
          {
            handoff_time_HANDOFF_RECV = now + ((double)((ta_change) * Random(state->ctx)));
            break;
          }
          case pcs_EXPONENTIAL:
          {
            handoff_time_HANDOFF_RECV = now + ((double)((Expent(state->ctx, ta_change))));
            break;
          }
          default: {
            handoff_time_HANDOFF_RECV = now + ((double)((5 * Random(state->ctx))));
          }
        }
...
struct simulation_configuration conf = {
  ...
  .gvt_period = 1000,
  .log_level = LOG_INFO,
  .stats_file = "pcs_stats",
  .ckpt_interval = 0,
  .core_binding = true,
  .serial = false,
  .dispatcher = ProcessEvent,
  .committed = CanEnd,
  .termination_time = 600.0,
};
