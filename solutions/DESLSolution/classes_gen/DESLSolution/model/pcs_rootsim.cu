...
#define HANDOFF_LEAVE 5
#define FADING_RECHECK 6
...
#define WHAT_CLASS(id) ((id >= 0 && id <= 39999) ? classA : classUnknown)
...
static double generate_cross_path_gain( ctx)
{
  ...
  variation = 10 * random(ctx, g_n_nodes);
...
static void fading_recheck(struct lp_state_type *state,  ctx)
{
  ...
  while(ch != NULL) {
    ch->sir_data->fading = random_expctx, 1.0);
    ch = list_next(ch);
  }
...
;
    ...
    {
      c->sir_data->fading = random_expctx, 1.0);
...
__device__
void ProcessEventGPUclassA(lp_id_t me, simtime_t now, unsigned event_type, const void *content, struct lp_state_type* state)
{
...
;
       ...
       */
      double timestamp_LP_INIT = ((double)((20 * random(&(nodes.cr_state[nid]), g_n_nodes))));
      
      
      /* 
       * start the first fading recheck
       */
      timestamp_LP_INIT = ((double)((pcs_FADING_RECHECK_FREQUENCY * random(&(nodes.cr_state[nid]), g_n_nodes))));
    ...
    case START_CALL:
      ...
      else
      {
        ...
        new_event_content_START_CALL.channel = allocation(state, &(nodes.cr_state[nid]));
        ...
        switch (pcs_DURATION_DISTRIBUTION)
        {
          case pcs_UNIFORM:
          {
            new_event_content_START_CALL.call_term_time = now + ((double)((ta_duration * random(&(nodes.cr_state[nid]), g_n_nodes))));
            break;
          }
          case pcs_EXPONENTIAL:
          {
            new_event_content_START_CALL.call_term_time = now + ((double)((random_exp&(nodes.cr_state[nid]), ta_duration))));
            break;
          }
          default: {
            new_event_content_START_CALL.call_term_time = now + ((double)((5 * random(&(nodes.cr_state[nid]), g_n_nodes))));
          }
        }
        ...
        switch (pcs_CELL_CHANGE_DISTRIBUTION)
        {
          case pcs_UNIFORM:
          {
            handoff_time_START_CALL = now + ((double)((ta_change) * random(&(nodes.cr_state[nid]), g_n_nodes)));
            break;
          }
          case pcs_EXPONENTIAL:
          {
            handoff_time_START_CALL = now + ((double)((random_exp&(nodes.cr_state[nid]), ta_change))));
            break;
          }
          default: {
            handoff_time_START_CALL = now + ((double)((5 * random(&(nodes.cr_state[nid]), g_n_nodes))));
          }
        }
      ...
      switch (pcs_DISTRIBUTION)
      {
        case pcs_UNIFORM:
        {
          timestamp_START_CALL = now + ((double)((state->ta * random(&(nodes.cr_state[nid]), g_n_nodes))));
          break;
        }
        case pcs_EXPONENTIAL:
        {
          timestamp_START_CALL = now + ((double)((random_exp&(nodes.cr_state[nid]), state->ta))));
          break;
        }
        default: {
          timestamp_START_CALL = now + ((double)((5 * random(&(nodes.cr_state[nid]), g_n_nodes))));
        }
      }
    ...
    case HANDOFF_RECV:
      ...
      else
      {
        ...
        new_event_content_HANDOFF_RECV.channel = allocation(state, &(nodes.cr_state[nid]));
        ...
        switch (pcs_CELL_CHANGE_DISTRIBUTION)
        {
          case pcs_UNIFORM:
          {
            handoff_time_HANDOFF_RECV = now + ((double)((ta_change) * random(&(nodes.cr_state[nid]), g_n_nodes)));
            break;
          }
          case pcs_EXPONENTIAL:
          {
            handoff_time_HANDOFF_RECV = now + ((double)((random_exp&(nodes.cr_state[nid]), ta_change))));
            break;
          }
          default: {
            handoff_time_HANDOFF_RECV = now + ((double)((5 * random(&(nodes.cr_state[nid]), g_n_nodes))));
          }
        }
    ...
    case FADING_RECHECK:
      fading_recheck(state, &(nodes.cr_state[nid]));
...
void ProcessEventGPU(lp_id_t me, simtime_t now, unsigned event_type, const void *content, unsigned size, void *data)
{
  switch(WHAT_CLASS(me)) {
    ...
    case classA:
      ProcessEventGPUclassA(me, now, event_type, content, (struct lp_state_type *)data);
      break;
