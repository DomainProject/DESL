#!/bin/sh
# ---------------------------------------------------------------------
# Discrete Event Simulation Language startup script.
# ---------------------------------------------------------------------
# Generated by MPS
IDEA_PATHS_SELECTOR=DESL1.0

message()
{
  TITLE="Cannot start Discrete Event Simulation Language"
...
# Try (in order): $DESL_JDK, .../desl.jdk, .../jbr, .../jbr-x86, $JDK_HOME, $JAVA_HOME, "java" in $PATH.
# ---------------------------------------------------------------------
IDEA_VENDOR_NAME='NoCompany'
# shellcheck disable=SC2154
if [ -n "$DESL_JDK" ] && [ -x "$DESL_JDK/bin/java" ]; then
  JDK="$DESL_JDK"
fi

if [ -z "$JDK" ] && [ -s "${XDG_CONFIG_HOME:-$HOME/.config}/$IDEA_VENDOR_NAME/DESL1.0/desl.jdk" ]; then
  USER_JRE=$("$CAT" "${XDG_CONFIG_HOME:-$HOME/.config}/$IDEA_VENDOR_NAME/DESL1.0/desl.jdk")
...
if [ -z "$JDK" ] || [ ! -x "$JAVA_BIN" ]; then
  ...
  message "No JDK found. Please validate either DESL_JDK, JDK_HOME or JAVA_HOME environment variable points to valid JDK installation."
  fi
  exit 1
fi
...
# ---------------------------------------------------------------------
# shellcheck disable=SC2154
if [ -n "$DESL_PROPERTIES" ]; then
  IDE_PROPERTIES_PROPERTY="-Didea.properties.file=$DESL_PROPERTIES"
fi
...
if [ -n "$IDEA_VM_OPTIONS" ] && [ -r "$IDEA_VM_OPTIONS" ]; then
  ...
  VMOPTIONS_FNAME=desl${BITS}.vmoptions
  # 
...
# shellcheck disable=SC2154
if [ -n "$DESL_CLASSPATH" ]; then
  CLASSPATH="$CLASSPATH:$DESL_CLASSPATH"
fi
...
"$JAVA_BIN" \
  -classpath "$CLASSPATH" \
  ${VM_OPTIONS} \
  "-XX:ErrorFile=$HOME/java_error_in_DESL_%p.log" \
  "-XX:HeapDumpPath=$HOME/java_error_in_DESL.hprof" \
