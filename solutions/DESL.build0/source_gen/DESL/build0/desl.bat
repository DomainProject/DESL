...
:: Generated by MPS
::----------------------------------------------------------------------
SET IDEA_PATHS_SELECTOR=DESL1.0
SET PRODUCT=desl
...
IF "%IDE_BIN_DIR:~-8%" == "bin\win\" (
  echo.
  echo    You are trying to run Discrete Event Simulation Language from generic distribution
  echo Please first copy content of "%IDE_BIN_DIR%"
  echo into its parent directory ("%IDE_BIN_DIR:~0,-4%"^)
  echo and then run desl.bat from there
  echo.
  pause
  exit
)
...
:: Try (in order): DESL_JDK, desl%BITS%.exe.jdk, ..\jre, JDK_HOME, JAVA_HOME.
:: ---------------------------------------------------------------------
SET JDK=
SET IDEA_VENDOR_NAME=NoCompany

IF EXIST "%DESL_JDK%" SET "JDK=%DESL_JDK%"
IF EXIST "%JDK%" GOTO check

SET BITS=64
SET USER_JDK64_FILE=%APPDATA%\%IDEA_VENDOR_NAME%\DESL1.0\desl%BITS%.exe.jdk
SET BITS=
SET USER_JDK_FILE=%APPDATA%\%IDEA_VENDOR_NAME%\DESL1.0\desl%BITS%.exe.jdk
...
IF NOT EXIST "%JAVA_EXE%" (
  ECHO ERROR: cannot start Discrete Event Simulation Language.
  ECHO No JDK found. Please validate either DESL_JDK, JDK_HOME or JAVA_HOME points to valid JDK installation.
  EXIT /B
)
...
:: ---------------------------------------------------------------------
IF NOT "%DESL_PROPERTIES%" == "" SET IDE_PROPERTIES_PROPERTY="-Didea.properties.file=%DESL_PROPERTIES%"

SET VM_OPTIONS_FILE=
SET USER_VM_OPTIONS_FILE=
IF NOT "%DESL_VM_OPTIONS%" == "" (
  :: 1. %<IDE_NAME>_VM_OPTIONS%
  IF EXIST "%DESL_VM_OPTIONS%" SET "VM_OPTIONS_FILE=%DESL_VM_OPTIONS%"
)
