...
public class ElseIfPart_TextGen extends TextGenDescriptorBase {
  ...
  public void generateText(final TextGenContext ctx) {
    ...
    if (SPropertyOperations.getBoolean(ctx.getPrimaryInput(), PROPS.isForward$pAg5) || !(requiresReversibility)) {
      ...
      for (SNode stmt : ListSequence.fromList(SLinkOperations.getChildren(SLinkOperations.getTarget(ctx.getPrimaryInput(), LINKS.body$_xRC), LINKS.statements$IdM8))) {
        tgs.indent();
        tgs.appendNode(stmt);
        tgs.newLine();
      }
      if (requiresReversibility) {
        if (isContainedInLoop) {
          ...
          tgs.append(SPropertyOperations.getString(SLinkOperations.getTarget(SNodeOperations.cast(SNodeOperations.getParent(ctx.getPrimaryInput()), CONCEPTS.IfStatement$AR), LINKS.variable$WrxR), PROPS.name$MnvL));
          tgs.append(" |=  (1 << ");
          tgs.append(String.valueOf(SNodeOperations.getIndexInParent(ctx.getPrimaryInput()) + 1));
          tgs.append(");");
          tgs.newLine();
