    ...
    return new AbstractCellAction() {
      ...
      public void execute_internal(EditorContext editorContext, SNode node) {
        if (SNodeOperations.hasRole(node, LINKS.iterator$fwes)) {
          List<SNode> additionalIterators = SLinkOperations.getChildren(SNodeOperations.getNodeAncestor(node, CONCEPTS.ForStatement$Bb, false, false), LINKS.additionalIterators$umBI);
          if (ListSequence.fromList(additionalIterators).isNotEmpty()) {
            SNode first = SNodeOperations.cast(ListSequence.fromList(additionalIterators).first(), CONCEPTS.ForVarDecl$3i);
