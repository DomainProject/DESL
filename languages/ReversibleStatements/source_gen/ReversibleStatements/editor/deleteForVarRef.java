  ...
  /*package*/ static AbstractCellAction createAction_DELETE(final SNode node) {
    return new AbstractCellAction() {
      ...
      public void execute_internal(EditorContext editorContext, SNode node) {
        if (SNodeOperations.hasRole(node, LINKS.iterator$fwes)) {
          SNode forStatement = SNodeOperations.getNodeAncestor(node, CONCEPTS.ForStatement$Bb, false, false);
          List<SNode> additionalIterators = SLinkOperations.getChildren(forStatement, LINKS.additionalIterators$umBI);

          if (ListSequence.fromList(additionalIterators).isNotEmpty()) {
            SNode firstIterator = SNodeOperations.cast(ListSequence.fromList(additionalIterators).first(), CONCEPTS.ForVarRef$v6);
            ...
            SelectionUtil.selectNode(editorContext, SLinkOperations.getTarget(copyIterator, LINKS.init$llBv));
            return;
          }
        }
